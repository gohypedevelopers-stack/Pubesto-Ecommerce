...
const CUSTOMERS_FILE = process.env.PUBESTO_CUSTOMERS_FILE
  ? path.resolve(process.env.PUBESTO_CUSTOMERS_FILE)
...
function sanitizeCustomer(customer) {
  ...
  return {
    ...
    authProvider: customer.googleId ? "google" : "local",
...
function normalizeCustomer(customer) {
  ...
  return {
    ...
    googleId: String(customer.googleId || ""),
...
export async function upsertGoogleCustomer({ googleId, name, email, avatarUrl, shopifyCustomerId }) {
  const normalizedEmail = normalizeEmail(email);
  const customers = await readCustomersRaw();
  ...
  if (!normalizedEmail || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(normalizedEmail)) {
    throw new Error("Google account email is missing or invalid.");
  }

  const index = customers.findIndex((customer) => (
    customer.googleId === String(googleId) ||
  ...
  if (index >= 0) {
    ...
    const nextCustomer = normalizeCustomer({
      ...
      name: existing.name && existing.name !== "Pubesto Customer"
        ? existing.name
        : String(name || existing.name || "Pubesto Customer").trim(),

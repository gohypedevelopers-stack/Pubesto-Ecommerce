...
export function fixShopifyCheckoutUrl(url) {
  // Return the original Shopify URL to ensure checkout works correctly
  // Replacing with customDomain was causing 404s because the custom domain isn't hosting the Shopify checkout
  return url;
}
...
export function getShopifyVariantIdForColor(productSlug, colorName) {
  ...
  if (slug.includes("neck-fan")) {
    if (color.includes("white") || color.includes("arctic")) {
      return "gid://shopify/ProductVariant/51699939508440";
    }
    if (color.includes("pink") || color.includes("blush")) {
      return "gid://shopify/ProductVariant/51699472203992";
    }
    if (color.includes("blue")) {
      return "gid://shopify/ProductVariant/5169947236760" || "gid://shopify/ProductVariant/51699472236760";
    }
...
export function getShopifyCartPermalink(items = []) {
  ...
  let permalink = getShopifyStorefrontUrl(`/cart/${lines.join(",")}`);

  const totalQty = expandedItems.reduce((sum, item) => sum + item.quantity, 0);
  if (totalQty === 2) {
    permalink += "?discount=PACKOF2";
  } else if (totalQty === 4) {
    permalink += "?discount=PACKOF4";
  }
...
export async function getShopifyProducts() {
  try {
    ...
    return data.products.edges.map(({ node }) => ({
      ...
      image: node.images.edges[0]?.node.url || "https://placehold.co/600x600/1b624b/ffffff?text=Pubesto",
...
export async function getShopifyCheckoutUrl(handle, quantity = 1) {
  ...
  let finalUrl = fixShopifyCheckoutUrl(checkoutUrl);
  if (quantity === 2) {
